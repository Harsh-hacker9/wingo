...
const addWinGo = async (game) => {
    try {
        ...
        let period = winGoNow.length > 0 ? winGoNow[0].period : '2022070110000'; // cầu hiện tại - use default if no data
        ...
        if (minPlayers.length >= 2) {
            ...
            `);
                return { name: column.name, total_money: result.length > 0 && result[0].total_money ? parseInt(result[0].total_money) : 0 };
            });
        ...
        // Check if period already exists to prevent duplicate entry
        const [existingRecord] = await connection.query('SELECT * FROM wingo WHERE period = ? AND game = ?', [Number(period) + 1, join]);
        ...
        if (existingRecord.length === 0) {
            ...
            await connection.execute('UPDATE wingo SET amount = ?, status = ?, time = ? WHERE period = ? AND game = ?', [0, 0, timeNow, Number(period) + 1, join]);
        }
