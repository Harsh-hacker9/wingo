...
const register = async (req, res) => {
    ...
    // Set default invite code if none provided
    if (!invitecode) {
        // Use the admin's code as default if available, otherwise use a placeholder
        const [admin] = await connection.query('SELECT code FROM users WHERE name_user = "Admin" LIMIT 1');
        if (admin.length > 0) {
            invitecode = admin[0].code;
        } else {
            // If no admin found, try to get any existing user's code
            const [user] = await connection.query('SELECT code FROM users WHERE code IS NOT NULL AND code != "" LIMIT 1');
            if (user.length > 0) {
                invitecode = user[0].code;
            } else {
                invitecode = '00000'; // Fallback default
            }
        }
    }
    ...
    try {
        ...
        // Only check invite code if it's not the default '00000' value
        let check_i = [];
        if (invitecode !== '00000') {
        ...
        if (check_u.length == 1 && check_u[0].veri == 1) {
            ...
            // If default invite code, proceed with registration
            if (invitecode === '00000' || check_i.length == 1) {
                if (check_ip.length <= 3) {
                    let ctv = '';
                    
                    // If default invite code, set ctv to empty string, otherwise use referrer's ctv
                    if (invitecode !== '00000') {
                    ...
                    // Only update referrer's level if not using default invite code
                    if (invitecode !== '00000') {
